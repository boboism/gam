class SparepartTransDocItemIn < TransDocItem
  ...
  belongs_to :trans_doc, class_name: "SparepartTransDocIn", foreign_key: "trans_doc_id"

  before_save :update_no_and_name
  def update_no_and_name
    temp_sparepart = Sparepart.where(id: sparepart_id).first
    self.sparepart_no, self.sparepart_name = temp_sparepart.try(:part_no), temp_sparepart.try(:part_name) 
  end
